...
function changes(v){
    ...
    var ss1=document.getElementById(ss[v]);
    with(ss1){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        ...
        if(++v<ss.length){changes(v);}
    }//End with
}
function changess(v){
    var str="0";
    for(i=0;i<v;i++){
        str+=("_"+(document.getElementById(sss[i]).selectedIndex-1));
    };
    var ss1=document.getElementById(sss[v]);
    with(ss1){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(sss[v-1]).selectedIndex>0 || !v){
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++){
                    options[length]=new Option(ar[i],ar[i]);
        ...
        if(++v<sss.length){changess(v);}
    }//End with
}
function changesss(v){
    var str="0";
    for(i=0;i<v;i++){
        str+=("_"+(document.getElementById(ssss[i]).selectedIndex-1));
    };
    var ss1=document.getElementById(ssss[v]);
    with(ss1){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(ssss[v-1]).selectedIndex>0 || !v){
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++){
                    options[length]=new Option(ar[i],ar[i]);
...
var ss=["sss_province","sss_city","sss_county"];//三个select的name
var sss=["ss_province","ss_city","ss_county"];//三个select的name
var ssss=["ssss_province","ssss_city","ssss_county"];//三个select的name
...
function _init_area1(){  //初始化函数
    for(i=0;i<ss.length-1;i++){
      document.getElementById(ss[i]).onchange=new Function("changes("+(i+1)+")");
    }
    changes(0);
}
function _init_area2(){  //初始化函数
    for(i=0;i<sss.length-1;i++){
      document.getElementById(sss[i]).onchange=new Function("changess("+(i+1)+")");
    }
    changess(0);
}
function _init_area3(){  //初始化函数
    for(i=0;i<ssss.length-1;i++){
      document.getElementById(ssss[i]).onchange=new Function("changesss("+(i+1)+")");
    }
    changesss(0);
}